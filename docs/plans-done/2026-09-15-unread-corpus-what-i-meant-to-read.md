...
## Follow-on tasks (Adam, 2026-09-16)

1. **The Matter queue joins the corpus.** Matter is where reading happens now; Instapaper is a historical snapshot. The existing Matter client (`scripts/matter/api.py`, status `queue`) returned 510 unread items on 2026-09-16, about 1.3M words, median 1,405 words, with a 2022 cohort of 163 never cleared and 201 items touched in 2026. Pull queue content through the same client (no fetch chain), enrich with the same base schema plus the unread template, and merge into the same outputs so "what I meant to read" is one record with two sources: Instapaper (frozen, 492) and Matter (live, 510). The Matter half carries `reading_progress`, so "started, never finished" is separable from "never opened." Runs after the pipeline PR lands.
2. **The backlog, as a living view, Matter only.** A nightly queue snapshot leg on the Matter sync (count, ids, word counts, per-item reading_progress) into a ledger, so inflow (new queue ids per week), outflow (ids leaving the queue: read from the archive sync's date_read, or removed), and net drain are measured rather than inferred. The hero stats: queue size today, week-over-week change, inflow per week, reads per week, net drain, and a burn-down projection that states its assumption (median net drain over the trailing N weeks) and is allowed to say "never at this rate." Backfill: the archive's `date_read` history gives reads per week for two years; `updated_at` gives inflow floors; the May 2023 export is one hard past snapshot (431 unread by the queue definition - Saved and not Read; the raw Read == False count of 487 included 56 inbox items) for the long-run line. Mockup first (a section of the Reading record, in that record's own design), then build; the snapshot leg starts now so the ledger accumulates while the mockup waits.

> Shipped 2026-09-16: #28 (pipeline, 492 enriched on gemini-2.5-flash-lite, $0.23) and #29 (record mockup, approved by Adam the same night). Follow-ups: the four aggregates into public_shape(), the live-web probe pass (question 6), the parking-page guard, and the data.adamthede build (dispatched).
