...
- **Matter integration — second read-it-later source** — **built, in review** (P1). Adam reads in two apps: Instapaper (since ~2008) and Matter (recent years), and Matter content was stranded — no local copies, no enrichment, not in the dashboard. `scripts/core/export_matter_to_archive.py` now pulls items and highlights from Matter's public API v1 into the same Markdown + YAML frontmatter the rest of the archive uses, with incremental `updated_since` sync, cross-era URL dedupe, and a nightly launchd job; the proven enrich → index → dashboard pipeline picks it up unchanged. Full contract in `docs/MATTER_SYNC.md`. `next: Adam generates a token at web.getmatter.com/settings, writes it to ~/.secrets/matter.token, then runs --check-auth and --dry-run` `depends: an active Matter Pro subscription (required for API access)`
...
- **Content-type handling for non-articles** — Matter brings content types the Instapaper pipeline never saw (podcasts, videos, PDFs, tweets, newsletters); Instapaper was articles-only. Confirm the enrichment prompt handles non-article text gracefully and surface the `content_type` frontmatter field as a dashboard filter — the Matter sync already writes `content_type` and `build_index.py` now carries it into the Parquet index, so the filter is the remaining piece. `effort: ~half day`
- **Unified article-sync daemon (launchd)** — the Matter half is done (`launchd/com.thedetech.article-sync.matter.plist`, nightly 04:45, syncs then rebuilds the index). What remains is folding in Instapaper's incremental pull and the Gemini enrichment pass, so one nightly job covers the whole pipeline. This is the "always-on local agent" pattern (shared with the PKM enrichment daemon) applied to reading history. `next: after the Instapaper incremental import works by hand`
...
- **Reading-era provenance in the index** — `build_index.py` now records `source`, `content_type` and `matter_id` per article, and the dashboard groups the corpus into Instapaper / Legacy / Matter eras with a sidebar filter. Also fixed the index output path: after `build_index.py` moved into `scripts/core/`, it had been writing to `scripts/data/` while the dashboard read `data/`, so every rebuild needed a manual copy.
