...
import unread_pages as unread_mod  # noqa: E402
...
def main(argv=None):
    ...
    # The unread pages (study step 6). Defaults are the files the nightly legs
    # already write, so the nightly rebuild regenerates these with no new
    # arguments on the plist.
    ap.add_argument("--unread-enriched", default=str(unread_mod.DEFAULT_ENRICHED),
                    help="Enriched unread corpus (JSONL) for /unread/")
    ap.add_argument("--matter-daily", default=str(unread_mod.DEFAULT_DAILY_CSV),
                    help="Matter queue ledger, one row per night")
    ap.add_argument("--matter-ledger", default=str(unread_mod.DEFAULT_LEDGER),
                    help="Matter queue snapshots (JSONL); only the last line is read")
    ap.add_argument("--no-unread", action="store_true",
                    help="Skip the private /unread/ pages")
    ...
    unread_paths = None if args.no_unread else {
        "enriched": args.unread_enriched, "daily_csv": args.matter_daily,
        "ledger": args.matter_ledger}
    ...
    unread_built = [p for p in ("unread", "unread/versus", "unread/worth")
                    if (out / p / "index.html").exists()]
    if unread_built:
        extra += f", {len(unread_built)} unread pages"
    if payload.exists():
